...
describe('UserService', () => {
  ...
  describe('register', () => {
    it('should register a new user successfully', async () => {
      ...
      UserRepository.create.mockResolvedValue({ _id: '64a7b8c9d1e2f3a4b5c6d7e8', username: 'testuser', email: 'test@example.com', password: 'hashedPassword' });
      ...
      // Assert
      expect(validateInput).toHaveBeenCalledWith(registerDto.username, registerDto.email, registerDto.password);
      expect(emailValidator).toHaveBeenCalledWith(registerDto.email);
      expect(UserRepository.findByUsernameOrEmail).toHaveBeenCalledWith(registerDto.username, registerDto.email);
    ...
    it('should throw an error if validation fails', async () => {
      ...
      const registerDto = { username: '', email: 'test@example.com', password: 'Password123!' };

      await expect(UserService.register(registerDto)).rejects.toThrow('Validation failed: username is required');
    });

    it('should throw an error if email is invalid', async () => {
      validateInput.mockReturnValue([]);
      emailValidator.mockReturnValue('Invalid email');
      const registerDto = { username: 'testuser', email: 'bademail', password: 'Password123!' };

      await expect(UserService.register(registerDto)).rejects.toThrow('Invalid email');
    });

    it('should throw if user already exists', async () => {
      validateInput.mockReturnValue([]);
      emailValidator.mockReturnValue(null);
      UserRepository.findByUsernameOrEmail.mockResolvedValue({ _id: 'existingUserId' });
      const registerDto = { username: 'testuser', email: 'test@example.com', password: 'Password123!' };

      await expect(UserService.register(registerDto)).rejects.toThrow('User already exists with this email or username');
    });
  });
  ...
  describe('login', () => {
    it('should login successfully', async () => {
      ...
      expect(UserRepository.updateOnlineStatus).toHaveBeenCalledWith('user123', true);
    ...
    it('should throw if email or password is missing', async () => {
      await expect(UserService.login({ email: 'test@example.com' })).rejects.toThrow('Email and password are required');
      await expect(UserService.login({ password: 'pass' })).rejects.toThrow('Email and password are required');
    });

    it('should throw if email is invalid', async () => {
      emailValidator.mockReturnValue('Invalid email');
      await expect(UserService.login({ email: 'bademail', password: 'pass' })).rejects.toThrow('Invalid email');
    });

    it('should throw if user not found', async () => {
      emailValidator.mockReturnValue(null);
      UserRepository.findByEmail.mockResolvedValue(null);
      await expect(UserService.login({ email: 'test@example.com', password: 'pass' })).rejects.toThrow('Invalid credentials');
    });
    ...
    it('should throw if password is invalid', async () => {
      ...
      await expect(UserService.login({ email: 'test@example.com', password: 'wrongpass' })).rejects.toThrow('Invalid credentials');
    });
  });
  ...
  describe('logout', () => {
    it('should logout successfully', async () => {
      ...
      expect(UserRepository.updateOnlineStatus).toHaveBeenCalledWith('user123', false);
  ...
  describe('changePassword', () => {
    ...
    it('should throw if required fields are missing', async () => {
      await expect(UserService.changePassword('user123', { currentPassword: '' })).rejects.toThrow('Current password and new password are required');
    });

    it('should throw if new password validation fails', async () => {
      validatePassword.mockReturnValue(['too weak']);
      await expect(UserService.changePassword('user123', { currentPassword: 'old', newPassword: 'weak' }))
    ...
    it('should throw if user not found', async () => {
      ...
      await expect(UserService.changePassword('user123', { currentPassword: 'old', newPassword: 'NewPass123!' }))
    ...
    it('should throw if current password is incorrect', async () => {
      ...
      await expect(UserService.changePassword('user123', { currentPassword: 'wrongOld', newPassword: 'NewPass123!' }))
  ...
  describe('getAllUsers', () => {
    it('should return all users except current user', async () => {
      ...
      expect(UserRepository.getAllExceptUser).toHaveBeenCalledWith(currentUserId);
